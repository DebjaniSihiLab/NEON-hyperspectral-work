...
first=FALSE
if(first==TRUE){
  files <- list.files(paste0(user_wd,"/NEON-AOP/data/hs_lai/DP2.30012.001/"),
                      pattern = "3m_wide.csv$") 
  ...
  neon_hs_table <- 
    foreach (i = 1:length(files),.combine=rbind) %dopar% {
      temp_df <- read_csv(paste0(user_wd,"/NEON-AOP/data/hs_lai/DP2.30012.001/",
                                 files)[i])
  ...
  write_csv(neon_hs_table,
            paste0(user_wd,"/NEON-AOP/data/hs_lai/temporary_neon_lai_table.csv")
  )
}
#final results:------
neon_hs_table <- read_csv(paste0(user_wd,"/NEON-AOP/data/hs_lai/temporary_neon_lai_table.csv"))
...
ggplot()+
  geom_point(data=neon_hs_soc.df,aes(x=plotID, y=lai,group=plotID,
...
ggsave(file=paste0(user_wd,"/NEON-AOP/data/neon_lai_soc.png"),
