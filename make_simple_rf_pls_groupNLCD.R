...
wide_data=wide_data %>% select(-soilTypeOrder) %>% drop_na()
#nlcd group
wide_data$nlcdClass %>% unique()# %>% length()
# [1] "deciduousForest"            "evergreenForest"           
# [3] "mixedForest"                "woodyWetlands"             
# [5] "shrubScrub"                 "cultivatedCrops"           
# [7] "pastureHay"                 "emergentHerbaceousWetlands"
# [9] "grasslandHerbaceous"        "sedgeHerbaceous"           
# [11] "dwarfScrub"   
table(wide_data$nlcdClass)
# cultivatedCrops     deciduousForest          dwarfScrub     evergreenForest 
# 102                        94                  24                 356 
# grasslandHerbaceous         mixedForest          shrubScrub       woodyWetlands 
# 230                               43                 351                  25 
wide_data=wide_data %>% 
  ...
  filter(nlcdClass=="shrubScrub") %>% #R2=71.63%
  # filter(nlcdClass=="cultivatedCrops") %>%
  # filter(nlcdClass=="pastureHay") %>% 
  # filter(nlcdClass=="emergentHerbaceousWetlands") %>% 
  # filter(nlcdClass=="grasslandHerbaceous") %>%
  # filter(nlcdClass=="sedgeHerbaceous") %>% 
  # filter(nlcdClass=="dwarfScrub") %>%
  select(-nlcdClass)
dim(wide_data)
...
# 1. Save model
file_name <- "explore_models/deciduousForest_model.RData"#% Var explained: 26.91
file_name <- "explore_models/evergreenForest_model.RData"# % Var explained: 33.71
file_name <- "explore_models/mixedForest_model.RData"# % Var explained: -33.71
file_name <- "explore_models/woodyWetlands_model.RData" #% Var explained: 9.96
file_name <- "explore_models/shrubScrub_model.RData" # % Var explained: 71.63
file_name <- "explore_models/cultivatedCrops_model.RData"# % Var explained: -4.91
file_name <- "explore_models/grasslandHerbaceous_model.RData"#% Var explained: 13.53
file_name <- "explore_models/dwarfScrub_model.RData"#% Var explained: -29.12
