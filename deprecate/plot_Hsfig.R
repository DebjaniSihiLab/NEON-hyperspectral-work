...
list.files("DP3.30012.001/")
hs_df <- read_csv("DP3.30006.001/2019_D01_HARV_HS_windows3m.csv")
...
final_df <- left_join(hs_df,lai_df,by="plotID") %>% 
  left_join(soc_df,by="plotID") %>% 
...
final_df_mean <- final_df %>% group_by(OCrange,index) %>% 
...
ggsave(file="/Users/zhuonanwang/Library/CloudStorage/OneDrive-EmoryUniversity/01_Projects-sihi/NEON-AOP/data/hs_lai/RFigures/Hs_colorOC.png",fig,
