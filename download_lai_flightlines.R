...
library(neonUtilities) #byFileAOP
...
cat("DP2.30012.001---> download NEON-AOP flight line lai data, lai data: \n")  
...
HARV_spc <-  spc_mgp_sls_30cm_locationSOC %>% filter(siteID == site) %>%
  ...
  if(downloadAOPfile==TRUE){
    cat("download this site SOC plots  lai flightline \n")
    options(timeout=60*120)
    getOption('timeout')
  #set  check.size = F, so continously download
  #HS download site-year by flight line
  byFileAOP(dpID = "DP2.30012.001", # NEON-AOP lai flight line data DP1
              site = site, # Site code
...
#run function:
download_HS_flightlines(spc_mgp_sls_30cm_locationSOC,site="CPER",
                        downloadAOPfile=TRUE,year=2021)
download_HS_flightlines(spc_mgp_sls_30cm_locationSOC,site="RMNP",
                        downloadAOPfile=TRUE,year=2021)
download_HS_flightlines(spc_mgp_sls_30cm_locationSOC,site="STER",
                        downloadAOPfile=TRUE,year=2021)
