...
neonsites <- read_csv(paste0(user_wd,"neon_hs_sites_3ndRound.csv"))
neonsites$site %>% length()

for(a_site in 1:nrow(neonsites)){
  site=neonsites[[a_site,2]]
  siteyear=neonsites[neonsites$site==site,3][[1]]
  #run function:
  hdf5_cloud_condition(site=site,year=siteyear)
}
