...
soc_df=soc_df %>% filter(siteID=="CPER") #77
soc_df=soc_df %>% filter(siteID=="STER") #102
soc_df=soc_df %>% filter(siteID=="RMNP") #31
...
col_name <- paste0("wavelength_",colnames(wide_data)[-c(1,2,length(wide_data))])
length(col_name) #359 wavelengths/bands
colnames(wide_data)[-c(1,2,length(wide_data))] <- col_name
...
R2_tradi   <- caret::R2(pred = predicted_y, obs = y, form = "traditional")
...
png(file=paste0(user_wd,"/NEON-AOP/data/hs_lai/RFigures/gls_vip.png"),
...
ggplot( data = data.frame(obs = testing_set$OC.percent, 
  ...
  geom_smooth(aes(x=obs,y=pred),method = "lm")+
  xlab("observation")+ylab("prediction")+
  stat_regline_equation(label.y = 2.5,label.x = 11, aes(label = ..eq.label..)) +
  stat_regline_equation(label.y = 1.5, label.x = 11,aes(label = ..rr.label..))+
  xlim(0,8)+ylim(0,8) +                         
  geom_abline(intercept = 0,slope = 1)


ggsave(file=paste0(user_wd,"/NEON-AOP/data/hs_lai/RFigures/GLS_obs_pred_20percentOC.png"),
       width=5,height=5,dpi=400)  
