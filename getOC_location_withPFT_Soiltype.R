...
finalproduct <-paste0(user_wd,"/NEON-AOP/data/hs_lai/NEON_SOC_Locations/spc_mgp_sls_30cm_locationSOC_withPFT_Soiltype.csv")
...
source(paste0(user_wd,"/NEON-AOP/code/My_functions_R/spc_Cal_PitsLocation_withPFT_Soiltype.R"))
...
dim(sls_soilChem)#updated as 4222 from #3242  
...
sls30cm_locationSOC <- left_join(sls_soilChem_short,sls.loc_short,
  ...
  filter(!is.na(OC_percent)) #update to 2,975 from  #2282   16

sls_spatial_forjoin <- sls_spatial %>% 
  select( namedLocation,domainID,siteID,plotID,
          soilTypeOrder,elevation,slopeAspect,slopeGradient)
sls30cm_locationSOC <- left_join(sls30cm_locationSOC,sls_spatial_forjoin,
                                 by=c( "namedLocation","domainID","siteID","plotID")) 
...
aqp::site(sls) <- ~ domainID + siteID + plotID +collectDate+namedLocation+nlcdClass+sampleTiming+
  utmZone+adjNorthing+adjEasting+elevation.x+soilTypeOrder+slopeAspect+slopeGradient
...
mgp_spatial <- read.csv(paste0(dir, outputs, "/spatial/spatial_mgp.csv"), 
  ...
  select(namedLocation,locationType,domainID,siteID,
         ...
         elevation,nlcdClass,slopeAspect,slopeGradient) %>% 
  type_convert %>% 
...
for (i in 1:nrow(mgp30cm_df_utm)) {
  ...
  utmzone= as.numeric(gsub("N", "", mgp30cm_df_utm[i,14]))
  attr(latlong, "zone") <- utmzone 
  latlongUTM <- convUL(latlong, km=FALSE)
  mgp30cm_df_utm[i,15]=latlongUTM$X
  mgp30cm_df_utm[i,16]=latlongUTM$Y
}
...
write_csv(spc30cm_locationSOC,paste0(user_wd,"/NEON-AOP/data/hs_lai/NEON_SOC_Locations/spc30cm_locationSOC_withPFT_Soiltype.csv"))
write_csv(sls30cm_locationSOC,paste0(user_wd,"/NEON-AOP/data/hs_lai/NEON_SOC_Locations/sls30cm_locationSOC_withPFT_Soiltype.csv"))
write_csv(mgp30cm_locationSOC,paste0(user_wd,"/NEON-AOP/data/hs_lai/NEON_SOC_Locations/mgp30cm_locationSOC_withPFT_Soiltype.csv"))
...
temp_spc=spc30cm_locationSOC %>% 
  select(domainID,siteID,collectDate,wtmean.estimatedOC.percent,
         ...
         nlcdClass,elevation,slopeAspect,slopeGradient,
         soilTypeOrder)

temp_sls=sls30cm_locationSOC%>% 
  select(domainID,siteID,collectDate,wtmean.estimatedOC.percent,
         utmZone,piteasting=adjEasting,pitnorthing=adjNorthing,
         nlcdClass,elevation=elevation.x,slopeAspect,slopeGradient,
         soilTypeOrder) %>% 
  mutate(slopeAspect=as.numeric(slopeAspect),
         slopeGradient=as.numeric(slopeGradient))

temp_mgp=mgp30cm_locationSOC %>% 
  select(domainID,siteID,collectDate,wtmean.estimatedOC.percent,
         utmZone,piteasting=east,pitnorthing=north,
         nlcdClass,elevation,slopeAspect,slopeGradient)
temp_mgp$soilTypeOrder <- NA

combine <- bind_rows(temp_spc,temp_mgp,temp_sls) %>% 
  filter(!is.na(wtmean.estimatedOC.percent)) #3263   12
...
#FINAL products sls,spc,mgp 30cm soil SOC and location in UTM-----
write_csv(combine_clean,paste0(user_wd,"/NEON-AOP/data/hs_lai/NEON_SOC_Locations/spc_mgp_sls_30cm_locationSOC_withPFT_Soiltype.csv"))

#update ifelse(siteID == 'STEI'&utmZone=="15N","CHEQ",siteID))
...
stei16n=spc_mgp_sls_30cm_locationSOC %>% filter(siteID == "STEI") %>%
  arrange(rowID) %>% filter(utmZone=="16N")
stei15n=spc_mgp_sls_30cm_locationSOC %>% filter(siteID == "STEI") %>%
  arrange(rowID) %>% filter(utmZone=="15N")

spc_mgp_sls_30cm_locationSOC=spc_mgp_sls_30cm_locationSOC %>% 
  mutate(siteID = ifelse(siteID == 'STEI'&utmZone=="15N","CHEQ",siteID))

write_csv(spc_mgp_sls_30cm_locationSOC,paste0(user_wd,"/NEON-AOP/data/hs_lai/NEON_SOC_Locations/spc_mgp_sls_30cm_locationSOC_withPFT_Soiltype.csv"))

  test <- read_csv(finalproduct)
